...
struct grid {    
        ...
        int end[3];
        double length[3];
       // double dx,dy,dz; // moved to struct  cell 
       // double vol; 
        vector<array_struct_int_int_int> indx_1d;
        void set_up_global_grid(int n );
        void set_up_1d_indx();            
         };

extern grid global_grid;
extern grid local_r_grid;
extern grid local_k_grid;

extern int LOCAL_SIZE_K;

struct chemical {
    int n_spe;  // number of different chemical molecualr
    const int max_spe=3;  // three different molecualr at current version at most.
    double spe_kapa[max_spe]; // kapa value of each molecular, kapa controls the flexibility of each chemical molecular.
    double XN[max_spe][max_spe]; //the matrix of flory-huggins interaction parameters
    ...
    void set_up_chemical( int n , double *kapa, double **X_N);
...
struct chain {
   ...
   double block_kapa[4]; // kapa value of each block, kapa controls the flexibility of each block.
   int N_s;  // discretization of total contour length
   int Loa;  // kuhn segment length N=L/a
  ...
  // int N_Mbar;  // expansion rank number of sphere harmonics function
   ...
   void set_up_chain( int nn, int nb, double *fb, int *sp, chemical &Chemical);
   void clean_up_chain();
};
